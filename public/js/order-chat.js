...
(function () {
    ...
    const cancellationModal = get(".cancellation-modal")
    const milestoneInput = get("#milestone-input")
    const milestoneBTN = get(".release-milestone")
    const requestCancellationBTN = get(".request-cancellation")
    const closeRequestBTN = get(".close-request-modal")
    const deliveryModal = get(".delivery-modal")
    const deliveryModalBTN = get(".request-delivery")
    const closeDeliveryBTN = get(".close-delivery-modal")
    const disputeOrderBTN = get(".dispute-order")
    const disputeSlideOutContainer = get(".dispute-hide")
    const offerExtrasModal = get(".offer-extras-modal")
    const requestcancellationConfirmBTN = get("#request-cancellation")
    const acceptCancellationBTN = get("#accept-cancellation")
    const rejectCancellationBTN = document.getElementById("reject-cancellation")
    const messageController = get(".message-control")
    const sendextraBTN = get(".send-extra-offer")
    const extrasPriceInput = get(".extras-price")
    const extrasDaysInput = get(".extras-days")
    const extrasHeadingInput = get(".extras-heading")
    const sendDisputeBTN = get(".send-dispute")
    const disputeMsgInput = get("#dispute-message")
    const confirmDispute = get(".confirm-dispute")
    const closeDispute = get(".close-dispute-modal")
    const milestoneErrors = document.querySelectorAll(".milestone-error")
    let presentMilestoneAmount = get(".milestone-present")
    const milestoneSuccess = get(".milestone-success")
    const hiddenAtachment = get(".attachment")
    const attachBTN = get(".attach-btn")
    const fileNameHolder = get(".file-name-holder")
    const deliveryFilesHolder = get("#files")
    const deliveryFileList = get(".delivery-file-list")
    const deliveryMessage = get("#delivery-message")
    const deliveryCaution = get(".caution")
    const fileuploadError = get(".delivery-file-upload-error")
    const deliveryMessageError = get(".delivery-message-error")
    ...
    if (sendDelivery)
        sendDelivery.addEventListener("click", function () {
            ...
            if (!message) {
                deliveryMessageError.classList.remove("hide")
                isValidDelivery = false
            ...
            ajax.onload = () => {
                get(".upload-success").classList.remove("hide")
                get(".upload-progress").classList.add("hide")
                sendDelivery.classList.add("hide")
                get("#progressBar").classList.add("hide")
                deliveryFilesHolder.classList.add("hide")
                deliveryMessage.classList.add("hide")
                deliveryCaution.classList.add("hide")
