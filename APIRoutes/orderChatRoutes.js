...
router.post("/uploads", multerUpload.array("files"), async (req, res) => {
    try {
        const body = req.body
        console.log(req.files)
        const { from, to, message, order_id } = body
        console.log(body)
        const chat = new OrderChatModel({
            from,
            to,
            order_id,
            message,
            type: "delivery",
            content: {
                files: req.files.map(file => ({ name: file.filename, type: file.mimetype, size: file.size }))
